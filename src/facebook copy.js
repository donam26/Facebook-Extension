...
function writeCommentAndClosePopup() {
  const checkPopupInterval = setInterval(() => {
    ...
    if (popupDiv) {
          ...
          const checkCommentBoxInterval = setInterval(() => {
            ...
            if (divInputComment) {
              ...
              for (const commentBox of potentialCommentBoxes) {
                if (
                  commentBox &&
                  isElementVisible(commentBox) &&
                  !commentBox.hasAttribute("data-commented")
                ) {
              ...
              if (foundCommentBox) {
                foundCommentBox.setAttribute("data-commented", "true"); // Mark this comment box as processed
      ...
      let commented = false; // Flag to ensure we comment only once
      ...
      for (const field of directCommentFields) {
        if (
          ...
          !field.hasAttribute("data-commented") && // Check if already commented
          !commented // Ensure only one comment is made
        ) {
          const commentBox = field.querySelector(
            ".xzsf02u.x1a2a7pz.x1n2onr6.x14wi4xw.notranslate"
          );

          if (commentBox && !commented) {
            console.log("Found comment box:", commentBox);

            commentBox.setAttribute("data-commented", "true"); // Mark this comment box as processed
            ...
            // Insert the spun comment only once
            const spunComment = "This is the spun comment"; // Replace with actual spun comment logic
            document.execCommand("insertText", false, spunComment);
            commentBox.dispatchEvent(new Event("input", { bubbles: true }));
            commented = true; // Set the flag to true after commenting

            const sendButton = field.querySelector('[aria-label="Bình luận"]');
            if (sendButton) {
              sendButton.click();
              console.log("Đã nhấn vào nút gửi bình luận.");

              // Wait a bit before moving to the next post
              setTimeout(() => {
                isCommenting = false; // Reset the flag to allow next actions
                console.log("Proceeding to next post...");
              }, 3000); // Adjust the time delay as necessary

              break; // Exit the loop after commenting on the first field
            } else {
              console.log("Không tìm thấy nút gửi bình luận.");
              isCommenting = false; // Ensure the flag is reset if no send button found
            }
          } else {
            console.log("Không tìm thấy ô nhập bình luận.");
            isCommenting = false; // Reset if no comment box is found
          }
        }
      }
    }
